    ...
    <section className="mx-auto grid max-w-6xl gap-10 px-6 py-14 md:grid-cols-2 md:items-center md:py-20">
      ...
      <div>
        ...
        <div className="mt-8 grid gap-3 sm:grid-cols-3">
          {BRAND.kpis.map((x, idx) => (
            <Reveal key={x.label} delay={0.18 + idx * 0.06}>
              <Card className="rounded-2xl border-white/10 bg-white/5 text-white transition hover:bg-white/[0.07] hover:-translate-y-0.5 hover:shadow-[0_0_0_1px_rgba(255,255,255,0.08)]">
        ...
        <Reveal delay={0.4}>
          <div className="mt-6 rounded-2xl border border-white/10 bg-slate-900/30 p-4 text-sm text-white/70">
            <span className="font-semibold text-white">Transparencia:</span> te explicamos requisitos
            y ruta por ciudad antes de ejecutar. Si algo requiere presencialidad, también te lo
            decimos.
          </div>
        </Reveal>
      </div>
      ...
      <Reveal delay={0.12}>
        <Card
          id="como-funciona"
          className="rounded-3xl border-white/10 bg-white/5 text-white shadow-[0_0_0_1px_rgba(255,255,255,0.05)] backdrop-blur"
        >
          <CardContent className="p-6">
            ...
            <ol className="mt-6 space-y-3">
              {[
                ...
                <li
                  key={t}
                  className="group flex gap-3 rounded-2xl border border-white/10 bg-slate-900/40 p-4 transition hover:bg-slate-900/55 hover:-translate-y-0.5"
                >
